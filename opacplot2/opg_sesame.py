...
class OpgSesame:
    ...
    def parse(self):
        ...
        while True:
            ...
            if not matid in self.data:
                self.data[matid] = {}

            if self.verbose and (recid > 104):
                print(
                    "Material = %8i  Record = %8i  Entries = %8i"
                    % (matid, recid, nentries)
                )
            ...
            if (600 > recid) and (recid >= 500):
                print(
                    "Warning: No reader for SESAME opacity tables. The "
    ...
    def parseInfo(self, nentries, matid, recid):
        ...
        if self.verbose:
            print("  zbar = %g  abar = %g  rho0 = %g" % (words[0], words[1], words[2]))
    ...
    def parseEos(self, nentries, matid, recid):
        ...
        prefixes = {301: "total_", 303: "ioncc_", 304: "ele_", 305: "ion_", 306: "cc_"}
        ...
        self.data[matid][prefix + "pres"] = (
            words[start : start + ntemp * ndens].reshape((ntemp, ndens)).transpose()
            * GPA_TO_ERGCC
        )
        start = start + ntemp * ndens

        # Read specific internal energy array (in MJ/kg and convert to ergs/g):
        self.data[matid][prefix + "eint"] = (
            words[start : start + ntemp * ndens].reshape((ntemp, ndens)).transpose()
            * MJKG_TO_ERGCC
        )
        start = start + ntemp * ndens
    ...
    def toEosDict(
        ...
        try:
            ...
            raise ImportError(
                "You need the hedp module. You can get it here: "
        ...
        opp_ses_data["idens"] = (opp.NA * opp_ses_data["ele_dens"]) / opp_ses_data[
            "abar"
        ]
        ...
        if Anum is None:
            opp_ses_data["Anum"] = np.array(
                [ptab.elements[el].mass for el in opp_ses_data["Znum"]]
            )
        else:
            opp_ses_data["Anum"] = np.array(Anum)
        opp_ses_data["Zsymb"] = np.array(
            [ptab.elements[el].symbol for el in opp_ses_data["Znum"]], dtype="|S2"
        )
        ...
        # turn this off later. just for testing sesame writer
        ...
        if not "zbar" in opp_ses_data.keys():
            ...
            dens_arr, temp_arr = np.meshgrid(
                opp_ses_data["ele_dens"], opp_ses_data["ele_temps"]
            )
            zbar = eos.thomas_fermi_ionization(
                dens_arr, temp_arr, opp_ses_data["Znum"].mean(), opp_ses_data["abar"]
            ).T
...
def writeSesameFile(fn, t201, t301, t303, t304, t305, t502, t504, t505, t601):
    ...
    def theader(num):
        return (
            " 1  9999   %d   240   r    82803    22704   1                                 1\n"
            % num
        )
