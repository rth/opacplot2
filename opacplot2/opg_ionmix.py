...
class OpacIonmix:
    ...
    def __init__(self, fn, mpi, twot=False, man=False, hassele=False, verbose=False):
        ...
        if self.man == False:
            ...
            self.numDens = np.logspace(
                self.dens0_log10,
                self.dens0_log10 + self.ddens_log10 * (self.ndens - 1),
                self.ndens,
            )

            self.temps = np.logspace(
                self.temp0_log10,
...
def writeIonmixFile(
    ...
    if zbar is None:
        zbar = np.zeros((ndens, ntemps))
    if dzdt is None:
        dzdt = np.zeros((ndens, ntemps))
    if pion is None:
        pion = np.zeros((ndens, ntemps))
    if pele is None:
        pele = np.zeros((ndens, ntemps))
    if dpidt is None:
        dpidt = np.zeros((ndens, ntemps))
    if dpedt is None:
        dpedt = np.zeros((ndens, ntemps))
    if eion is None:
        eion = np.zeros((ndens, ntemps))
    if eele is None:
        eele = np.zeros((ndens, ntemps))
    if cvion is None:
        cvion = np.zeros((ndens, ntemps))
    if cvele is None:
        cvele = np.zeros((ndens, ntemps))
    if deidn is None:
        deidn = np.zeros((ndens, ntemps))
    if deedn is None:
        deedn = np.zeros((ndens, ntemps))

    if ngroups is None:
        ngroups = 0
    if opac_bounds is None:
        opac_bounds = (0.0, 1.0)
    if rosseland is None:
        rosseland = np.zeros((ndens, ntemps, ngroups))
    if planck_absorb is None:
        planck_absorb = np.zeros((ndens, ntemps, ngroups))
    if planck_emiss is None:
        planck_emiss = np.zeros((ndens, ntemps, ngroups))
    ...
    for tab in [
        ...
        if ctab.shape != (ndens, ntemps):
            raise ValueError(
                "Table {0} has shape {1}, expected {2}!".format(
    ...
    def convert(num, name):
        ...
        try:
            ...
            raise ValueError(
                "There was a problem writing the data in "
    ...
    if sele is not None:
        write_block(sele.flatten() * ERG_TO_JOULE, "electron entropy")
